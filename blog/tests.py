...
class BlogTests(TestCase):
    ...
    def setUp(self):
        ...
        self.post = Post.objects.create(
            ...
            author=self.user,
            body='Just a casual body that I am testing with',
        )
    
    def test_string_rep(self):
        post = Post(title='A sample title for my string test')
    ...
    def test_post_content(self):
        self.assertEqual(f'{self.post.title}', 'This is a test title')
        self.assertEqual(f'{self.post.author}', 'testuser')
        self.assertEqual(f'{self.post.body}', 'Just a casual body that I am testing with')

    def test_post_list_view(self):
        response = self.client.get(reverse('home'))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Just a casual body that I am testing with')
